...
class SaveCopyDatabaseAction extends AbstractAction {
	private static final long serialVersionUID = 1L;
	private final Logger log = LoggerFactory.getLogger(getClass().getSimpleName());
	private final Gui gui;
	private final DatabaseManager dbm;
	
	SaveCopyDatabaseAction(Gui gui, DatabaseManager dbm, EventSupport eventSupport) {
		this.gui = gui;
		this.dbm = dbm;
		I18n.setText(this, "ACTION_SAVE_COPY");
	...
	public void actionPerformed(ActionEvent e) {
		if(dbm.isDatabaseOpen()) {
			File copyFile = gui.showSaveDialog();
			if(copyFile != null) {
				try {
